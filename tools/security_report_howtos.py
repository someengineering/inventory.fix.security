...
This guide assumes that you have already [installed](../../getting-started/install/index.mdx) and configured Fix Inventory to [collect your AWS cloud resources](../../getting-started/configure/aws.mdx).

## Directions

1. Execute the following [`search` command](../../reference/cli/search.mdx) in [Fix Inventory Shell](../../reference/components/shell.mdx):

   ```bash
   > {command}
   # highlight-start
   ​kind={result_kind}, ..., region=fixinventory-poweruser
