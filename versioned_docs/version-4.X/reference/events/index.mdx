...
Fix Inventory ships with [built-in cleanup plugins](../components/plugins/index.mdx) that can be enabled in the `fixinventory.worker` configuration.
...
| `cleanup_plan`      | Cleanup [plugins](../components/plugins/index.mdx) bundled with Fix Inventory listen on this event. |
