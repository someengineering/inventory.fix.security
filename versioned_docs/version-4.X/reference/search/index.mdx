...
Fix Inventory allows for [merging nodes](./merging-nodes.mdx), which can be a useful in case the information is not part of the node itself but in a reachable node nearby. It also allows filtering results with a certain graph structure using the [with clause](./with-clause.mdx).

<DocCardList />

## Searching Snapshots

You can search a specific Fix Inventory Rewind [asset inventory graph](../../concepts/asset-inventory-graph/index.mdx) snapshot by prefixing a search with the `graph=<graph-name>` filter:
...
Alternatively, you can provide the `--at` flag to the [`search` command](../cli/search.mdx) to search at a specific point in time. Fix Inventory will find the [asset inventory graph](../../concepts/asset-inventory-graph/index.mdx) snapshot that is closest to the specified time.
