...
While Fix Inventory has knowledge of cloud-specific resource kinds like [`aws_ec2_volume`](aws.mdx#aws_ec2_volume) and [`gcp_disk`](gcp.mdx#gcp_disk), both of those kinds inherit from the base `volume` kind, which in turn inherits from the [`resource` base kind](index.mdx#resource-base-kind):
