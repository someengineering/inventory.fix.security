...
The shape and format of the object can be adjusted with commands such as [`list`](./list.mdx), [`format`](./format.mdx), [`jq`](./jq.mdx), etc.

:::tip

You can use the [`chunk`](./chunk.mdx) command to send chunks of objects:
