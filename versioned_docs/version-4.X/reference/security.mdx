...
**By default, all Fix Inventory [components](./components/index.mdx) communicate with each other via HTTPS using Transport Layer Security (TLS).**
...
[Fix Inventory Core](./components/core.mdx) runs a Public Key Infrastructure (PKI) including a Certificate Authority (CA). Upon start, all components will request a certificate from the CA.

## Pre-Shared Key (PSK) \{#pre-shared-key}

Each Fix Inventory [component](./components/index.mdx) accepts a `--psk` flag (which can alternatively be supplied using the environment variables `FIXCORE_PSK`, `FIXWORKER_PSK`, `FIXMETRICS_PSK` and `FIXSHELL_PSK`).
...
If a [<abbr title="pre-shared key">PSK</abbr>](#pre-shared-key) is provided, every request that a component makes to [Fix Inventory Core](./components/core.mdx) must provide a valid `Authentication` header with a <abbr title="JSON web token">JWT</abbr> signed using the <abbr title="pre-shared key">PSK</abbr>.
...
Alternatively, a custom CA cert and key can be provided to [Fix Inventory Core](./components/core.mdx) using the `--ca-cert` and `--ca-cert-key` flags.
...
[Fix Inventory Core](./components/core.mdx) has two API endpoints, [`/ca/cert`](./api/retrieve-the-certificate-authorities-public-certificate.api.mdx) and [`/ca/sign`](./api/sign-a-certificate-request.api.mdx).

The former serves the core's public CA root certificate. Upon startup, all other components will request this root certificate from [Fix Inventory Core](./components/core.mdx) without validating the TLS connection.

However, [Fix Inventory Core](./components/core.mdx) encodes the certificate's SHA256 fingerprint into a <abbr title="JSON web token">JWT</abbr> which is signed with the previously mentioned <abbr title="pre-shared key">PSK</abbr>.
...
Any HTTPS requests between components and [Fix Inventory Core](./components/core.mdx) from this point forward are validated against the CA root certificate.
...
[Fix Inventory Shell](./components/shell.mdx) can be used to create custom certificates. This is useful for securing the connection to other components like [ArangoDB](https://arangodb.com) or [Prometheus](https://prometheus.io).

To create a certificate, open [Fix Inventory Shell](./components/shell.mdx) and execute:
...
When interfacing with Fix Inventory's [API endpoints](./api/index.mdx), it is possible to integrate with [Fix Inventory Core](./components/core.mdx) using the same transport encryption and authentication utilized by Fix Inventory's [components](./components/index.mdx).

### Retrieving and Validating the CA Certificate

The Fix Inventory CA certificate can be retrieved at [https://localhost:8900/ca/cert](https://localhost:8900/ca/cert) (replace `localhost` with the hostname or IP where [Fix Inventory Core](./components/core.mdx) is running).
...
$ fixinventory_command="search is(resource) | count"
$ curl --cacert fixinventory_ca.crt -H "$auth_header" -H "Content-Type: text/plain" -H "Accept: application/json" -X POST -d "$fixinventory_command" https://localhost:8900/cli/execute
```
...
Fix Inventory's authentication infrastructure is based on the <abbr title="pre-shared key">PSK</abbr> provided to [Fix Inventory Core](./components/core.mdx) on startup.

If a <abbr title="pre-shared key">PSK</abbr> is not provided, authentication is not required to access Fix Inventory's [API endpoints](./api/index.mdx).

### Disabling TLS

[Fix Inventory Core](./components/core.mdx) can be started without TLS using the `--no-tls` flag.
