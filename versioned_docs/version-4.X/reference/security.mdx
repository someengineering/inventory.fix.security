...
Each Fix Inventory [component](./components/index.mdx) accepts a `--psk` flag (which can alternatively be supplied using the environment variables `RESOTOCORE_PSK`, `fixworker_PSK`, `fixmetrics_PSK` and `fixshell_PSK`).
