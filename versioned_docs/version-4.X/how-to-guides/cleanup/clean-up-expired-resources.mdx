...
5. [Create an event-based job](../automation/create-an-event-based-job.mdx) to run the app automatically:
   ...
   The [`cleanup-expired` infrastructure app](../../reference/infrastructure-apps/cleanup-expired.mdx) will now run each time Fix Inventory emits the `post_cleanup_plan` [event](../../reference/events/index.mdx). The `post_cleanup_plan` event is emitted in the [`cleanup` phase](../../reference/workflows/index.mdx#cleanup) of the [`collect_and_cleanup` workflow](../../reference/workflows/index.mdx#collect_and_cleanup-workflow).
