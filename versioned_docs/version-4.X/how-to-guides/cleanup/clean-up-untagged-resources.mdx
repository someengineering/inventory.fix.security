...
Fix Inventory's [`cleanup_untagged` plugin](../../reference/components/plugins/cleanup_untagged.mdx) can enforce tagging policies by automatically cleaning up resources that do not have required tags (e.g., `owner` and `expiration`).
...
3. Update the `plugin_cleanup_untagged` section with the desired target AWS account IDs and setting the `enabled` property to `true`:
   ...
   - `tags` lists tags that **must** exist on every [resource kind](../../concepts/asset-inventory-graph/index.mdx#resource-kinds) listed in the `kinds` subsection.
   - `kinds` lists [resource kind](../../concepts/asset-inventory-graph/index.mdx#resource-kinds) for which tags listed in `tags` **must** exist.
...
- [`cleanup_untagged` Plugin](../../reference/components/plugins/cleanup_untagged.mdx)
