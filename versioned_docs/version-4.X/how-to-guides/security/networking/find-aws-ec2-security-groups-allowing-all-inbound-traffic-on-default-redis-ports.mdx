---
sidebar_label: Find AWS EC2 Security Groups Allowing All Inbound Traffic on Default Redis Ports
---

# How to Find AWS EC2 Security Groups Allowing All Inbound Traffic on Default Redis Ports
...
1. Execute the following [`search` command](../../../reference/cli/search.mdx) in [Fix Inventory Shell](../../../reference/components/shell.mdx):
   ...
   ```bash
   > search is(aws_ec2_security_group) and group_ip_permissions[*].{(ip_protocol=-1 or (from_port>=6379 and to_port<=6379 and ip_protocol=tcp)) and (ip_ranges[*].cidr_ip="0.0.0.0/0" or ipv6_ranges[*].cidr_ipv6="::/0")}
   # highlight-start
...
2. Pipe the `search` command into the [`dump` command](../../../reference/cli/dump.mdx):
   ...
   ```bash
   > search is(aws_ec2_security_group) and group_ip_permissions[*].{(ip_protocol=-1 or (from_port>=6379 and to_port<=6379 and ip_protocol=tcp)) and (ip_ranges[*].cidr_ip="0.0.0.0/0" or ipv6_ranges[*].cidr_ipv6="::/0")} | dump
   # highlight-start
   ​reported:
   ​  id: /aws/ec2/123
   ​  name: some-name
