...
Fix Inventory constantly monitors your infrastructure, and can alert you to any detected issues. [PagerDuty](https://pagerduty.com) is the de-facto standard to escalate alerts. In this guide, we will configure Fix Inventory to send alerts to PagerDuty with a [custom command](../../../reference/cli/index.mdx).
...
4. Add the routing key copied in step 2 as the default value of the `routing_key` parameter in the `pagerduty` section. This will allow you to execute the `pagerduty` command without specifying the routing key parameter each time.
   ...
   | `source`       | Alert source                                                                                     | `Fix Inventory`                           |
...
7. Finally, we want to automate checking of the defined alert trigger and send alerts to [PagerDuty](https://pagerduty.com) whenever the result is true. We can accomplish this by creating a [job](../../../concepts/automation/index.mdx#jobs):
