...
1. Execute the following [`search` command](../../../reference/cli/search.mdx) in [Fix Inventory Shell](../../../reference/components/shell.mdx):
   ...
   # highlight-start
   ​kind=aws_kms_key, ..., region=fixinventory-poweruser
...
2. Pipe the `search` command into the [`dump` command](../../../reference/cli/dump.mdx):
   ...
   The command output will list the details of all non-compliant [`aws_kms_key` resources](../../../reference/unified-data-model/aws/kms/aws_kms_key.mdx).
...
- [`aws_kms_key` Resource Data Model](../../../reference/unified-data-model/aws/kms/aws_kms_key.mdx)
