...
1. Execute the following [`search` command](../../../reference/cli/search.mdx) in [Fix Inventory Shell](../../../reference/components/shell.mdx):
   ...
   # highlight-start
   ​kind=aws_apigateway_rest_api, ..., region=fixinventory-poweruser
...
2. Pipe the `search` command into the [`dump` command](../../../reference/cli/dump.mdx):
   ...
   The command output will list the details of all non-compliant [`aws_apigateway_rest_api` resources](../../../reference/unified-data-model/aws/apigateway/aws_apigateway_rest_api.mdx).
...
- [`aws_apigateway_rest_api` Resource Data Model](../../../reference/unified-data-model/aws/apigateway/aws_apigateway_rest_api.mdx)
