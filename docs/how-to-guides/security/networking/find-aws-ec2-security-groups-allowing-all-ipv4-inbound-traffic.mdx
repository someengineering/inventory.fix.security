---
sidebar_label: Find AWS EC2 Security Groups Allowing All IPv4 Inbound Traffic
---

# How to Find AWS EC2 Security Groups Allowing All IPv4 Inbound Traffic
...
1. Execute the following [`search` command](../../../reference/cli/search.mdx) in [Fix Inventory Shell](../../../reference/components/shell.mdx):
   ...
   ```bash
   > search is(aws_ec2_security_group) and group_ip_permissions[*].{ip_protocol="-1" and ip_ranges[*].cidr_ip="0.0.0.0/0"}
   # highlight-start
...
2. Pipe the `search` command into the [`dump` command](../../../reference/cli/dump.mdx):
   ...
   ```bash
   > search is(aws_ec2_security_group) and group_ip_permissions[*].{ip_protocol="-1" and ip_ranges[*].cidr_ip="0.0.0.0/0"} | dump
   # highlight-start
   ​reported:
   ​  id: /aws/ec2/123
   ​  name: some-name
