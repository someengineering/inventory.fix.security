...
1. Execute the following [`search` command](../../../reference/cli/search.mdx) in [Fix Inventory Shell](../../../reference/components/shell.mdx):
   ...
   # highlight-start
   ​kind=aws_ec2_volume, ..., region=fixinventory-poweruser
...
2. Pipe the `search` command into the [`dump` command](../../../reference/cli/dump.mdx):
   ...
   The command output will list the details of all non-compliant [`aws_ec2_volume` resources](../../../reference/unified-data-model/aws.mdx#aws_ec2_volume).
...
- [`aws_ec2_volume` Resource Data Model](../../../reference/unified-data-model/aws.mdx#aws_ec2_volume)
