...
1. Execute the following [`search` command](../../../reference/cli/search.mdx) in [Fix Inventory Shell](../../../reference/components/shell.mdx):
   ...
   # highlight-start
   ​kind=aws_ec2_snapshot, ..., region=fixinventory-poweruser
...
2. Pipe the `search` command into the [`dump` command](../../../reference/cli/dump.mdx):
   ...
   The command output will list the details of all non-compliant [`aws_ec2_snapshot` resources](../../../reference/unified-data-model/aws/ec2/aws_ec2_snapshot.mdx).
...
- [`aws_ec2_snapshot` Resource Data Model](../../../reference/unified-data-model/aws/ec2/aws_ec2_snapshot.mdx)
