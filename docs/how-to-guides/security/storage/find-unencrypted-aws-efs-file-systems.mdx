...
1. Execute the following [`search` command](../../../reference/cli/search.mdx) in [Fix Inventory Shell](../../../reference/components/shell.mdx):
   ...
   # highlight-start
   ​kind=aws_efs_file_system, ..., region=fixinventory-poweruser
...
2. Pipe the `search` command into the [`dump` command](../../../reference/cli/dump.mdx):
   ...
   The command output will list the details of all non-compliant [`aws_efs_file_system` resources](../../../reference/unified-data-model/aws/efs/aws_efs_file_system.mdx).
...
- [`aws_efs_file_system` Resource Data Model](../../../reference/unified-data-model/aws/efs/aws_efs_file_system.mdx)
