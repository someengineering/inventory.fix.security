...
## Selecting Nodes by [Kind](../../concepts/asset-inventory-graph/index.mdx#resource-kinds) \{#selecting-nodes-by-kind}

```bash title="Filter by kind"
> search is(instance)
```

In order to select [nodes](../../concepts/asset-inventory-graph/index.mdx#nodes) by a specific type, the search syntax supports the `is(<kind>)` function. The term `is(instance)` would select the EC2 instance above, but also all other instances, e.g. Google Cloud instances, while the term `is(aws_ec2_instance)` would select only EC2 instances from AWS.
...
Fix Inventory will add a synthetic `id` property to each [node](../../concepts/asset-inventory-graph/index.mdx#nodes) that is globally unique, no matter which cloud provider you are using. Nodes can be selected by their internal ID via the `id(xyz)` function.
...
In order to filter for specific attributes of a [node](../../concepts/asset-inventory-graph/index.mdx#nodes), it is possible to define predicates. A predicate always has the syntax `<property_path> <operation> <value>` (e.g., `answer != 42`).
...
Every resource in Fix Inventory is represented by a JSON document with the following structure:
