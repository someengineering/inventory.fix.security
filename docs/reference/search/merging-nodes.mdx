...
Most cloud resources are maintained in an account. Accounts are modeled as [nodes](../../concepts/asset-inventory-graph/index.mdx#nodes) in Fix Inventory.
...
Fix Inventory provides the `ancestors` and `descendants` section as part of every resource to make this a little easier.
...
- It starts with a `/`. Fix Inventory interprets property paths relative to the `reported` section by default. Since we want to access a property on the object root level, we need to prefix the path with a `/`.
...
It is possible to access every property of every parent or child resource in this way. You can use this notation anywhere a property path is specified: in [filters](./filters.mdx), [aggregations](./aggregation.mdx), [`with` clauses](./with-clause.mdx), and [CLI commands](../../reference/cli/index.mdx). Fix Inventory is clever enough to create nested searches on demand when it encocunters this notation.
...
Fix Inventory provides the following information as part of every resource node.
...
The [command-line interface](../../reference/cli/index.mdx) uses `list` for outputs by default, which always prints the above data if available.
