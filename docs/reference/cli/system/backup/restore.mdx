...
pagination_label: system backup restore
---
...
Restoring a backup acquires a global write lock. This means that no writes can be performed while the backup is being restored. After the restore process is done, the `fixcore` process will stop and be restarted by the process supervisor automatically.
