...
While Fix Inventory has knowledge of cloud-specific resource kinds like [`aws_ec2_volume`](./aws/ec2/aws_ec2_volume.mdx) and [`gcp_disk`](./gcp/compute/gcp_disk.mdx), both of those kinds inherit from the base `volume` kind, which in turn inherits from the [`resource` base kind](base/index.mdx):
