...
This plugin looks for resources with the tags `expiration` or `fix:expires` and flags them for cleanup if they are expired.
...
| `fix:expiration` | `24h`                       | A timedelta relative to the resource's creation time |
