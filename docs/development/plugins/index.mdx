...
Plugin source code lives in the [`plugins` directory within the `someengineering/fixinventory` GitHub repository](https://github.com/someengineering/fixinventory/tree/main/plugins). Each plugin is maintained as separate project.

**Contributions are made via [pull requests to the GitHub repository](https://github.com/someengineering/fixinventory/pulls).** You will first need to [fork](https://docs.github.com/get-started/quickstart/fork-a-repo) the repository.

Pull requests should target a single plugin. Please refer to [Contributing to Components](../components.mdx) for details on how to clone the repository, set up a virtual environment, start Fix Inventory, and submit your changes for review.
...
Once the graph data is collected and sent to [Fix Inventory Core](../../reference/components/core.mdx), the power of Fix Inventory's [search](../../reference/search/index.mdx) is at your fingertips.
...
Fix Inventory uses a thin wrapper on top of [NetworkX](https://networkx.org) to operate on graphs. The two most used methods in the wrapper are `add_resource` and `add_edge`.
...
Please refer to the [example collector plugin](https://github.com/someengineering/fixinventory/tree/main/plugins/example_collector) for a model of how to implement this logic.

### Testing

To test a collector plugin, simply launch Fix Inventory and trigger the collect action manually by executing `workflow run collect` in [Fix Inventory Shell](../../reference/components/shell.mdx).
...
- **Don't forget to call `sanitize(graph)` before checking its properties in tests.**
  ...
  Please refer to the [DigitalOcean plugin](https://github.com/someengineering/fixinventory/tree/main/plugins/digitalocean) as an example.
