...
Fix Inventory has built-in action plugins for [resource management](../resource-management/index.mdx) that also run in this phase of the cloud data sync process.

These plugins can be activated via the [Fix Inventory Worker configuration](../../reference/configuration/worker.mdx).
