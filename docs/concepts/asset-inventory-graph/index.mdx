...
Fix Inventory scrapes resources from your cloud providers. This data is the source of truth for all of Fix Inventory's features. All resources from all cloud providers, accounts, regions and all resource types become nodes in the graph.

Fix Inventory also knows how resources relate to each other. For example, a compute instance might have a volume and is attached to a network interface. The same compute instance might be the target of a load balancer, which is inside a security group, which itself is inside a VPC. All of these relationships are encoded as edges in the graph.

The superpower of Fix Inventory is the ability to provide a complete picture of your cloud assets and how they relate to each other.
...
| `name`   | string   | The name of the resource. Fix Inventory tries its best to map the relevant property for that matter.             |
...
There is a plethora of resource types. Fix Inventory captures all of this information and makes it available to you.

To ensure you get consistent data, Fix Inventory defines a set of resource kinds. Each resource type is mapped to one of these kinds. Fix Inventory ensures that the data you get for a specific kind adheres to the schema defined for that kind.
...
The [`kinds` command](../../reference/cli/kinds.mdx) allows you to list all resource kinds and their properties.

:::

## Graph Edges \{#edges}

Fix Inventory also captures the relationships between resources by encoding the relationships as edges in the graph. This makes it possible to traverse the graph and walk from one resource to another based on a specific relationship. The relationship itself does not have any additional properties.

The list of possible edges are also defined explicitly in our [data model reference section](../../reference/unified-data-model/index.mdx). Another option is the [`kinds`](../../reference/cli/kinds.mdx) command to get the same information on the command line.
...
In order to traverse the graph in a meaningful way, it is important to understand the structure. The following diagram serves as an example graph you will find in Fix Inventory to illustrates how we can "walk" edges in a graph:
...
- [`ancestors` Command](../../reference/cli/ancestors.mdx)
- [`descendants` Command](../../reference/cli/descendants.mdx)
- [`predecessors` Command](../../reference/cli/predecessors.mdx)
- [`successors` Command](../../reference/cli/successors.mdx)
