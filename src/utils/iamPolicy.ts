...
export type AwsPolicyName = 'FixOrgList' | 'FixCollect' | 'FixMutate';
export interface AwsPolicy {
  Version: string;
  Statement: { Effect: string; Resource: string; Action: string[] }[];
}

export type GcpPolicyName = 'fix_access' | 'fix_mutate';
...
export const policyUrl = (
  ...
  return directory
    ? `https://cdn.some.engineering/fix/${provider}/${directory}/${name}.${
